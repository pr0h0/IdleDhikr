...
export default function EditPreset() {
  ...
  useEffect(() => {
    if (preset) {
      const itemsWithIds = preset.items.map((item) => ({
        ...item,
        id: item.id && item.id !== item.itemId ? item.id : generateId(),
      }));
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      >
        {items.map((item) => {
          // We need a stable unique ID for reordering. Since item.itemId might be duplicated
          // (if we allowed duplicate items, though current logic doesn't explicitly prevent it),
          // usually we'd want a unique instance ID.
          // For now, let's assume composition of itemId + idx or trust itemId if unique.
          // However, if we reorder, idx changes. If we have duplicate items, this key must be unique per item instance.
          // But `item` object reference is stable in local state array?
          // Framer Motion uses the item as valid key if it's an object, or we need a unique string key.
          // Let's assume using the item itself as value works for now.
          // A safer bet is to wrap items in an object with a random ID if duplicates were allowed.
          // Given current app, itemId is likely unique per preset (enforced? no).
          // Let's trust Framer Motion to handle object equality if references are stable.
          // Actually, Reorder.Item requires 'value' to be the item from the list, and 'key' to be stable unique.
          // We'll use a combination key or just itemId if unique.
